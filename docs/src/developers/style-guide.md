...
!!! info
    The style guide is always a work in progress, and not all MolecularDynamics code
    follows the rules. When modifying MolecularDynamics, please fix the style violations
...
MolecularDynamics uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/MolecularDynamics.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the MolecularDynamics directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to MolecularDynamics have
