...
[`MolecularDynamics`](https://github.com/singularitti/MolecularDynamics.jl).
...
version of `MolecularDynamics`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/MolecularDynamics.jl"
```

in the second step above.

## Update `MolecularDynamics`
...
our [GitHub repository](https://github.com/singularitti/MolecularDynamics.jl)
for new releases.
Once we release a new version, you can update `MolecularDynamics` by typing

```@repl
using Pkg
Pkg.update("MolecularDynamics")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `MolecularDynamics`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("MolecularDynamics")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `MolecularDynamics`.
